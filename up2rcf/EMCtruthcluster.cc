...
EMCtruthcluster::EMCtruthcluster(const std::string &name, const std::string &file):
 SubsysReco(name),
 _outfilename(file),
 _outfile(nullptr),
 _tree(nullptr)
{
  std::cout << "EMCtruthcluster::EMCtruthcluster(const std::string &name) Calling ctor" << std::endl;
}

//____________________________________________________________________________..
EMCtruthcluster::~EMCtruthcluster()
{
  std::cout << "EMCtruthcluster::~EMCtruthcluster() Calling dtor" << std::endl;
}

//____________________________________________________________________________..
int EMCtruthcluster::Init(PHCompositeNode *topNode)
{
  std::cout << topNode << std::endl;
  std::cout << "EMCtruthcluster::Init(PHCompositeNode *topNode) Initializing" << std::endl;
...
//____________________________________________________________________________..
void EMCtruthcluster::createBranches()
{
  delete _tree;
...
//____________________________________________________________________________..
int EMCtruthcluster::process_event(PHCompositeNode *topNode)
{
  std::cout<<"EMCtruthcluster::process_event event "<<cnt<<std::endl;
  cnt++;
  ...
  if (!clustersEM)
  {
    ...
    if (!clustersEM)
    {
      std::cout << "EMCtruthcluster::process_event: cannot find cluster container " << m_RawClusCont_EM_name << std::endl;
    }
  }
  ...
  if(!EMCAL_Container)
  {
    ...
    if(!EMCAL_Container)
    {
      std::cout << "EMCtruthcluster::process_event: TOWERINFO_CALIB_CEMC not found!!!" << std::endl;
    }
  }
  ...
  if(!EMCalGeo)
  {
    ...
    if(!EMCalGeo)
    {
      std::cout << "EMCtruthcluster::process_event: TOWERGEOM_CEMC not found!!!" << std::endl;
    }
  }
  ...
  if(!IHCalGeo)
  {
    ...
    if(!IHCalGeo)
    {
      std::cout << "EMCtruthcluster::process_event: TOWERGEOM_HCALIN not found!!!" << std::endl;
    }
  }
  ...
  if(!OHCalGeo)
  {
    ...
    if(!OHCalGeo)
    {
      std::cout << "EMCtruthcluster::process_event: TOWERGEOM_HCALOUT not found!!!" << std::endl;
    }
  }
  ...
  if(!vertexmap)
  {
    ...
    if(!vertexmap)
    {
      std::cout << "EMCtruthcluster::process_event: GlobalVertexMap not found!!! (but not necessary)" << std::endl;
    }
  }
  ...
  if (!truthinfo)
  {
    ...
    if(!truthinfo)
    {
      std::cout << "EMCtruthcluster::process_event: G4TruthInfo not found!!!" << std::endl;
    }
  }
...
//____________________________________________________________________________..
int EMCtruthcluster::End(PHCompositeNode *topNode)
{
  std::cout << topNode << std::endl;
  _outfile->cd();
  _outfile->Write();
...
void EMCtruthcluster::ResetTreeVectors()
{
  _emcal_id.clear();
  _emcal_phi.clear();
  _emcal_eta.clear();
